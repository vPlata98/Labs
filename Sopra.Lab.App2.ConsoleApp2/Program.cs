...
namespace Demo.Sopra.ConsoleApp1
{
    internal class Program
    {
        ...
        /// <summary>
        /// Busquedas basicas usando LINQ
        /// </summary>
        static void BusquedasBasicas()
        {
            //SELECT INSERT UPDATE DELETE
            //SELECT campos/columnas (*,todos)
            //FROM servidor, base de datos, esquema, tabla
            //WHERE condicion de seleccion
            //ORDER BY ordenacion
            //Con los consultables tenemos siempre la version mas actualizada

            //Seleccion de todos los campos de la lista de productos => SELECT * FROM ListaProductos
            ...
            var resultado1b = from r in DataLists.ListaProductos
                              where r.Precio > 2
                              orderby r.Precio
                              select new { r.Id, r.Precio }; //consultable

            foreach (var item in resultado1b) Console.WriteLine($"{item.Id} {item.Precio}");
        }

        static void BusquedasEjercicio()
        {
            // clientes nacidos entre 1980 y 1990
            // cliente mayores de 25 años
            ...
            var consulta3a = from c in DataLists.ListaProductos
                             orderby c.Precio descending
                             select c;
            Console.WriteLine($"Producto con el precio mas alto {consulta3a.FirstOrDefault().Id} {consulta3a.FirstOrDefault().Precio}");

            var consulta3b = DataLists.ListaProductos
                .OrderByDescending(p => p.Precio)
                .FirstOrDefault();
            Console.WriteLine($"Producto con el precio mas alto {consulta3b.Id} {consulta3b.Precio}");

            var consulta4a = (from p in DataLists.ListaProductos
                              orderby p.Precio
                              select p.Precio).Average();
            var consulta4b = DataLists.ListaProductos
                .Average(p => p.Precio);
            Console.WriteLine($"Media de precios {consulta4b}");

            var consulta5a = (from p in DataLists.ListaProductos
                              where p.Precio < consulta4b
                              select p).ToList();

            var consulta5b = DataLists.ListaProductos
                .Where(p => p.Precio < consulta4b);

            consulta5a.ForEach(p => Console.WriteLine($"Productos con precios inferiores a la media {p.Id} {p.Precio}"));
